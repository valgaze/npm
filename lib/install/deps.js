...
var realizeShrinkwrapSpecifier = limit(require('./realize-shrinkwrap-specifier'), 10)
var realizePackageSpecifier = limit(require('realize-package-specifier'), 10)
...
var recalculateMetadata = safe.recurseLimit(100, 10000, function (tree, log, seen, $$recurse$$, next) {
  validate('OOOFF', arguments)
  if (seen[tree.path]) return next()
  seen[tree.path] = true

  next = safe.onlyOnce(next)
  ...
  chain([
    ...
    [asyncMap, tree.children, function (child, done) { $$recurse$$(child, log, seen, done) }]
  ], function () {
...
exports.loadRequestedDeps = function (args, tree, saveToDependencies, log, next) {
  ...
  asyncMap(args, function (pkg, done) {
    var depLoaded = safe.onlyOnce(andAddParentToErrors(tree, done))
...
function andForEachChild (load, next) {
  validate('F', [next])
  return safe.onlyOnce(function (er, children, logs) {
...
var failedDependency = exports.failedDependency = safe.recurseLimitSync(100, 1000, function (tree, name_pkg, $$recurse$$) {
  var name
  var pkg = {}
  ...
  for (var ii = 0; ii < tree.requiredBy.length; ++ii) {
    ...
    if ($$recurse$$(requireParent, tree.package)) {
      return true
    }
  }
  return false
...
exports.loadDeps = safe.recurseLimit(100, 1000, loadDeps)
function loadDeps (tree, log, $$recurse$$, next) {
  ...
  }, andForEachChild($$recurse$$, andFinishTracker(log, next)))
})
...
var loadExtraneous = exports.loadExtraneous = function (tree, log, next) {
  var seen = {}
  var loadExtraneous = safe.recurseLimit(100, 1000, function (tree, log, $$recurse$$, next) {
    validate('OOFF', arguments)
    if (seen[tree.path]) return next()
    seen[tree.path] = true
...
var updatePhantomChildren = exports.updatePhantomChildren = function (current, child) {
  validate('OO', arguments)
  safe.whileSync(1000, function () { return current && current !== child.parent }, function () {
...
var resolveWithNewModule = limit(function $recurse$ (pkg, tree, log, next) {
  validate('OOOF', arguments)
...
var findRequirement = exports.findRequirement = safe.recurseLimitSync(100, 1000, function (tree, name, requested, requestor, $$recurse$$) {
  validate('OSOF', arguments)
  ...
  return $$recurse$$(tree.parent, name, requested, requestor)
})
...
var earliestInstallable = exports.earliestInstallable = safe.recurseLimitSync(100, 1000, function (requiredBy, tree, pkg, $$recurse$$) {
  validate('OOOF', arguments)
  ...
  return ($$recurse$$(requiredBy, tree.parent, pkg) || tree)
}
})
