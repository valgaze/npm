...
function addDependency (name, versionSpec, tree, log, done) {
  ...
  childDependencySpecifier(tree, name, versionSpec, iferr(done, function (req) {
    ...
    if (child) {
      ...
      fetchPackageMetadata(req, packageRelativePath(tree), log.newItem('fetchMetadata'), function (err, pkg) {
        if (err) {
          err.parent = moduleName(tree)
          err.parentLocation = path.relative(top(tree).path, tree.path)
          return next(err)
        }
